...
class VdcGW(VcdAnsibleModule):
    ...
    def _get_subnet_participation(self, gw_resource, network):
        for gateway_inf in gw_resource.Configuration.GatewayInterfaces.GatewayInterface:
    ...
    def update_sub_allocated_ip_pools(self):
        ...
        try:
            ...
            for network, new_ip_range in ip_pool.items():
                subnet_participation = self._get_subnet_participation(
                ...
                old_ip_range = "{0}-{1}".format(
                    ip_ranges.IpRange.StartAddress, ip_ranges.IpRange.EndAddress
                )
